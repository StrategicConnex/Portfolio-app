...
**P0 (2026-08-15) — design overhaul: paleta reducida + tipografía radical.** El audit de diseño experto (artista 3D + director creativo) identificó 3 trampas de "dark mode AI defaults": paleta plana (7 colores, todos fríos, amber amarillo Bootstrap), tipografía genérica (hero a 3rem, 5 tamaños similares) y glass genérico (blur 12px en todas las cards). **Paleta reducida a 6 semánticas:** `secondaryBlue` eliminado (consolidado a `primaryCold`), `securityAmber` de `#f59e0b` → `#c27a3a` (infrared industrial, NO amarillo), nuevo token `surface: #0d1520` para cards/glass. **Tipografía radical:** hero title de `clamp(1.5rem,7vw,3rem)` → `clamp(2.2rem,8vw,4.5rem)` / `clamp(3.5rem,7vw,7rem)` (de 3rem a 7rem, leading 0.95, tracking -0.04em), section titles de `clamp(1.9rem,3.8vw,3rem)` → `clamp(2rem,5vw,3.5rem)`. **CSS cleanup:** glass más sutil (blur 8px, bg surface), `.gradient-text` eliminado (genérico), `.glow-*` más sutiles (30px single shadow). 10 archivos tocados, cero cambios de runtime, gate clean (399/399, typecheck 0, lint 0, build OK) — [`report-p0-design-overhaul`](./reports/report-p0-design-overhaul.md).
